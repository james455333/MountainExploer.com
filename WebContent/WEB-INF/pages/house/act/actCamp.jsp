				...
				<nav>
					...
					<div class="third_nav" >
						...
						<form	action="<c:url value='/mountainCampAct/actselectAll'></c:url>"
							method='GET'>
							<div>鄉鎮: <select name="selectarea" id="selectCounties">
									<option>請選擇鄉鎮</option>
							</select>
			...
			<div class="secDivContent">
				...
				<table class="order-table">
					<thead class="order-table-th">
						...
						<c:out value="">${camp_all.name}</c:out>
				...
				<nav class="pageControl">
					...
					<ul>
						<li class="li1"><a href="actselectAll?selectarea=${selectarea }&s&no=${no }&page=1">«第一頁</a></li>
						<li class="li1"><a href="actselectAll?selectarea=${selectarea }&s&no=${no }&page=${page-1}">‹上一頁</a></li>
					<li class="li1">
					<select onChange="location = this.options[this.selectedIndex].value">
						<c:forEach var="toPage" begin="1" end="${totalPage}">
						<option value="actselectAll?selectarea=${selectarea }&s&no=${no }&page=${toPage}" <c:if test="${toPage==page}">selected="selected"</c:if>>第${toPage}頁</option>
						</c:forEach>

					</select></li>
						<li class="li1"><a href="actselectAll?selectarea=${selectarea }&s&no=${no }&page=${page+1}">下一頁›</a></li>
						<li class="li1"><a href="actselectAll?selectarea=${selectarea }&s&no=${no }&page=${totalPage}">最末頁»</a></li>
					</ul>
				</nav>
			</div>
